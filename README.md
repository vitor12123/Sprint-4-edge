# Sprint-4-edge

Integrantes: 
* Vitor Pallis RM: 561962
* Miguel Manfré RM: 564233
* Joao Toledo RM: 563433


pagina http: 
* http://20.57.132.118/


pagina wokwi:
* https://wokwi.com/projects/442196263369668609


Prints: 
<img width="1365" height="728" alt="image" src="https://github.com/user-attachments/assets/586cb08b-e944-48f9-b228-90e23c4d778a" />
<img width="1366" height="680" alt="image" src="https://github.com/user-attachments/assets/b6134483-c569-4f9d-970d-3def0b0480a6" />


Tecnologias usadas: 
* Sensor MRU6050
* Esp 32
* Html/css & javaScript
* Maquina virtual


Como Executar: 
1. Entre no link do wokwi e inicie o programa.
2. Espere o wokwi se conectar ao MQTT.
3. Entre na pagina http.


CONCLUSÃO: 
A PoC demonstra com sucesso a integração entre dispositivo edge, cloud computing e interface web, formando um sistema IoT completo e funcional.
